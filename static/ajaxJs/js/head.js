...
$(document).ready(function () {
	...
	let token = storage.getStorageData("token");

	if (token) {
		let userInfo = storage.getStorageData("userInfo");
		$(".SignIn").html(`<a href="edit-account" >Hello! ${userInfo.nickname}</a>`)
	}
...
const getInit = async () => {
	...
	await setGoodsCategoryList(category_list);
	...
	category_list.forEach(item => {
		...
		if (item.children && item.children.length) {
			item.children.forEach(listitem => {
				...
				itemhtml = itemhtml +
					`<li><a   href="${common.isSingleProduct() ?'../shop-list':'shop-list'}?id=${listitem.id}&title=` +
					name + `" >${listitem.name}</a></li>`
			})
		}
		name = item.name.replace(/ /g, "_");
		html = html + `<li>
			<a   class="menu-title"   href="${common.isSingleProduct()?'../shop-list':'shop-list'}?id=${item.id}&title=` +
			name + `" > ${item.name}</a>
															<ul>
																${itemhtml}
															</ul>
