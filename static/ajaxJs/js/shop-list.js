					...
					<div class="product-content">
						<div class="inner">
							...
							<div class="product-price-variant"><span
									...
									class="price old-price">$${item.price}</span>
							</div>
						</div>
					</div>
				</div>
...
const createPagination = (currentPage, totalPages) => {
	...
	if (currentPage > 1) {
		paginationHtml += `<li><a class="prev" href="#"><i class="la la-angle-left"></i></a></li>`;
	}
	...
	if (currentPage < totalPages) {
		paginationHtml += `<li><a class="next" href="#"><i class="la la-angle-right"></i></a></li>`;
	}
...
const generatePageNumberHtml = (pageNumber, currentPage) => {
	...
	return (
		`<li><a class="${pageNumber === currentPage ? "active" : ""}">${pageNumber}</a></li>`
...
$("#pagination").on("click", "li", function(event) {
	...
	if (pageNumber.includes("la-angle-left")) {
		selectPage(currentPage - 1);
	} else if (pageNumber.includes("la-angle-right")) {
		selectPage(currentPage + 1);
	} else {
